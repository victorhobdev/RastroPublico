...
from rastro_publico.coleta.arquivo import baixar_arquivo, manifestar_arquivo_existente
...
def test_manifesta_arquivo_existente_sem_reescrever_payload(tmp_path) -> None:
    arquivo = tmp_path / "compras.csv"
    arquivo.write_bytes(b"id,nome\n1,teste\n")

    manifesto = manifestar_arquivo_existente(
        arquivo=arquivo,
        url="https://dados.example/compras.csv",
        sistema_origem="comprasgov",
        dataset_origem="VW_FT_PNCP_COMPRA",
        run_id="run-existente",
        data_publicacao_arquivo="2026-07",
    )

    assert arquivo.read_bytes() == b"id,nome\n1,teste\n"
    documento = json.loads(manifesto.read_text(encoding="utf-8"))
    assert documento["tamanho_bytes"] == 16
    assert documento["hash_arquivo"] == (
        "c5011ec4d1ed9e0980d4d111e3d85fb036b001e6bb068b56adbc546dd313bf10"
    )
